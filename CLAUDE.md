...
| `components/navbar/Navbar.tsx` | Sticky top navigation | Desktop nav on `lg+`, hamburger mobile menu on `<lg`. Search bar with inline city picker, Events + Communities dropdowns, a **chat button** + notification bell (bell still a no-op), auth-aware avatar menu (My Dashboard, My Wishlist, My Organised Events [org only], Organiser Console, Settings, Log Out). The **chat button** (`ChatButton`, to the LEFT of the bell, authed only) opens `/chat` and, whenever any room has unread activity, **turns its icon green + shows a terracotta dot** (no animation — Gautham's call; an earlier pulsing green halo was removed), read from `useChatUnreadStore().unreadRooms`. The mobile hamburger has a matching **Messages** row with an unread dot. Detects organizer status via `organizerApi.get()` (5-min React Query cache). Includes the sun/moon **theme toggle** (desktop: after "Help"; mobile: a row in the hamburger panel) via `useTheme()`. |
...
- **⚠️ The ingest is FIRE-AND-FORGET and takes MINUTES — never `await` it in the endpoint.** A full city ingest measured **~186s** (it slices Ticketmaster by classification segment and paginates 200×5 within each, to beat the 1,000-result ceiling). The gateway proxies every route with a **30s** `timeout` ([`gateway/main.py`](backend/gateway/main.py)), so an endpoint that awaits the ingest blows through it on every single call. That surfaced as **`502 "Internal service recommendation is unreachable."`** — a badly misleading message, because the service was healthy and the ingest was *succeeding underneath* while the gateway had already hung up. The endpoint's docstring said "Returns immediately — ingestion runs in background" but line 58 awaited it; contract and implementation had silently diverged. Fixed by dispatching to `asyncio.create_task` via `_run_ingest()`, following the `_background_tasks` idiom `generate-events` already used in the same file. **Consequence: the 202 response no longer carries `created`/`upserted`/`failed` counts** — the work hasn't happened yet. Anything needing the outcome must poll `/event/search`, or read the recommendation service log, which is where `_run_ingest` reports it (the task is never awaited, so an exception would otherwise vanish silently). **That logging only works because `recommendation/main.py` now calls `logging.basicConfig(level=logging.INFO)`** — it previously did not (the gateway did), so the root logger had no handler and `logging.lastResort` silently dropped everything below WARNING. The completion line was invisible until that was added. **Most services still don't configure logging** — if you add an app-level `logger.info` to one and see nothing, this is why.
- **The gateway now returns `504`, not `502`, on a proxy timeout.** `httpx.TimeoutException` is a **subclass of `httpx.RequestError`**, so the original single `except httpx.RequestError` reported a slow-but-healthy service as an unreachable one. The `TimeoutException` handler must stay **above** the `RequestError` handler or it is unreachable and the bug returns.
- **Home page polls, it does not wait.** [`app/page.tsx`](frontend_react/apps/web/src/app/page.tsx) re-checks every `INGEST_POLL_MS` (10s) for up to `INGEST_WINDOW_MS` (240s), invalidating the events query so the grid fills as rows land, and only falls back to AI generation if the city is *still* empty when the window closes. The old code checked **once after 3 seconds** — correct when the call blocked until completion, wrong now, and it would have fired AI generation against every city that was merely still ingesting. The interval is cleared on unmount/city change.
